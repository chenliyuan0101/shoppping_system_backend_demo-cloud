...
import com.mall.marketing.support.GatewayAuthHeaders;
 ...
 *
 * <p>本类逐字沿用 mall-review / mall-user-center 的做法——这不是复制粘贴的偷懒，而是刻意的：
 * 身份模型必须是**全站同一套**，任何服务自作主张改一点，就会出现"某个服务能被冒充"的静默漏洞。
 *
 ...
 * {@code X-Member-Id: 1} 就能以别人的名义领券/用券（券是钱）。
 *
 * <p><b>三条纪律</b>：
 * <ol>
 ...
 * </ol>
 *
 * <p>⚠️ {@code /internal/**} 走的是**另一把钥匙**（{@link InternalApiAuthInterceptor} 的
 * {@code X-Internal-Token}）：内部端点的 {@code memberId} 是 trade 在服务端上下文里传进来的，
 * 不是网关身份头。两条链路必须分清，否则会出现"内部接口被前端直接调用"。
 *
...
public class GatewayIdentityResolver {
     ...
     *
     * <p>供"解析不到就回落别的维度"的调用方使用（批次的限流切面：按会员维度限流时解析不到身份，
     * 就回落到客户端 IP 计数）。
     */
