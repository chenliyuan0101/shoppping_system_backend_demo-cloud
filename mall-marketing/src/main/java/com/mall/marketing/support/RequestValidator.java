 ...
 * {@link BusinessException}(400, 原中文提示)。共享内核副本（与 review 逐字相同）。
 *
 ...
 * (内部调用、集成测试)时完全不生效**。所以约束写在 DTO 上，由 Service 在方法开头
 * 调 {@link #check(Object)} 触发，HTTP 与内部调用两条路径得到完全一致的结果。
 *
 * <p><b>报错顺序</b>：按 ①DTO 字段声明顺序 → ②同字段内注解优先级
 * （非空 → 长度 → 范围 → 格式）排序后取第一条——多字段同时非法时的先后顺序是契约的一部分。
 */
@Component
