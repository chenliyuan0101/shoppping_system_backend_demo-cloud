...
/**
 * Redis 缓存/计数的统一入口(本服务唯一出口)。
 *
 * <p>设计要点（与单体/其它服务逐字同一套）：
 *  1. **只用 StringRedisTemplate + JsonKit(Jackson 3)**：避开 GenericJackson2JsonRedisSerializer
 *     依赖 Jackson 2 的坑；
 ...
 *
 * <p>P4 第 1 批还没有任何缓存调用点（只读切片直接查库）：先把这个"唯一出口"放好，
 * 是为了评价列表/评论数的缓存不要在下一批各自手写一份 Redis 访问代码。
 */
@Slf4j
@Service
