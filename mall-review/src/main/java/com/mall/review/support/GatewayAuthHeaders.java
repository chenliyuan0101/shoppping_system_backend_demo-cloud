 ...
 *
 * <p>本服务是这三个头在下游的消费者之一：{@link com.mall.review.config.GatewayIdentityResolver}
 * 先校验 {@link #GATEWAY_AUTH}（共享密钥，常量时间比较、未配置即 fail-closed），
 * 只有通过之后 {@link #MEMBER_ID} 才被当作身份——否则任何人手写一个 {@code X-Member-Id: 1}
 * 就能冒充别人去提交/删除评价。
 *
 * <p>与单体 {@code com.mall.demo.common.GatewayAuthHeaders}、user-center 的同名类逐字相同
 * （三个头名必须一致，否则网关注入了身份而本服务认不出来）。
 */
