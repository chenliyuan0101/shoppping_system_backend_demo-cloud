...
import com.mall.review.support.GatewayAuthHeaders;
 ...
 *
 * <p>本类逐字沿用 mall-user-center 的做法（`config/GatewayIdentityResolver.java`）——
 * 这不是复制粘贴的偷懒，而是刻意的：身份模型必须是**全站同一套**，
 * 任何服务自作主张改一点，就会出现"某个服务能被冒充"的静默漏洞。
 *
 ...
 * {@code X-Member-Id: 1} 就能以别人的名义提交或删除评价。判据就是本类：
 * 请求头里的共享密钥 == 配置的密钥。
 *
 * <p><b>三条纪律</b>：
 * <ol>
 ...
 * </ol>
 *
 * <p><b>本类只回答"你是谁"，不回答"你的登录态还作不作数"</b>：令牌版本由网关比对，
 * 会员是否已被禁用由 user-center 的成员状态缓存体现——三件事分属不同层次。
 *
 * <p>配置项：{@code mall.gateway.auth-token}（dev 明文写在 profile，prod 走
 * {@code MALL_GATEWAY_AUTH_TOKEN}），必须与网关的同一项**逐字相同**；
 * 轮换时要网关与全部服务同时发布。
 */
@Slf4j
@Component
public class GatewayIdentityResolver {
     ...
     * 就回落到客户端 IP 计数——提交评价有限流，游客也可能在别处触发限流逻辑）。
     *
