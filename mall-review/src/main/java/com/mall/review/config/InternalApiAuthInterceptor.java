 ...
 * 抽服务时这些接口会变成 HTTP 端点。评价域的 {@code /internal/**} 会包含
 * "按订单明细查是否已评价""批量取某 SPU 的评论数"这类**被 trade/admin 调用**的能力；
 * 一旦暴露而没有鉴权，等于把"我的评价数据"公开出去（还能被刷计数）。
 *
 * <p>两道防线（缺一不可）：
 * <ol>
