...
/**
 * 收货地址的<b>只读契约</b>（供单体其它域使用，主要是下单链路）。
 *
 * <p>形状遵守《微服务改造方案.md》§2.8：只收发 DTO，不含 Entity / Mapper。
 * 归属校验（"这个地址是不是你的"）由属主域（现在是 user-center）完成——它是地址域的规则，
 * 不该让每个调用方各写一遍 `if (!address.getMemberId().equals(memberId))`。
 *
 * <p>说明：地址的<b>增删改</b>不在这里——那些是用户端接口（{@code /api/address/**}，
 * 已经由网关直接路由到 user-center），单体侧没有调用方，也不该有。
