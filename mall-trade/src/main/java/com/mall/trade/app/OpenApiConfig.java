 ...
 * 本服务**没有**登录端点（会员登录在 {@code mall-user-center}、管理端登录在 {@code mall-admin}，
 * 都经网关），因此这份文档里只有需要登录的业务接口。
...
public class OpenApiConfig {
    ...
    public OpenApiCustomizer exampleCustomizer() {
        return openApi -> {
            ...
            // ⚠️ P8-2a：原来这里还给 `/api/admin/auth/login` 注入请求体示例。
            //    该端点已不在本服务（P7 起 `/api/admin/auth/**` 由网关直路由 `mall-admin`），
            //    单体侧的登录控制器/服务/实体/mapper 已随"只信网关注入身份"一并删除
            //    ⇒ 这一段是**指向不存在路径**的文档装配，删掉而不是留着。
        };
    }
