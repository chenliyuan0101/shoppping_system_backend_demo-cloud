...
class TradeOutboxMySqlTest extends MySqlTestBase {
    ...
    void rawPayloadPathAlsoGoesThroughOutbox() {
        String biz = newBizKey();
        var payload = new com.mall.trade.oms.mq.OrderClosedMessage(biz, 4242L, 77L, com.mall.trade.oms.mq.OrderClosedMessage.REASON_CANCEL, System.currentTimeMillis());
        TransactionTemplate tx = new TransactionTemplate(txManager);
        tx.executeWithoutResult(status ->
