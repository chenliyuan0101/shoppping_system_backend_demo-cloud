...
/**
 * Redis 缓存/计数的统一入口(全项目唯一出口；本服务自持副本，与 user-center 逐字同构)。
 *
 * 设计要点：
 ...
 *     限流放行；并通过退避(30s)避免每次请求都去踩超时
 *  3. 可用 `mall.cache.enabled=false` 一键关闭全部 Redis 行为(保留纯 DB 逻辑)
 *
 * <p>⚠️ 管理端的 fail-open 是**明确要求**（P7 §2.5 与任务书）：状态缓存写不进去**绝不能**让管理员登不上后台。
 * 代价是"Redis 抖动期间 403 判据失效"——那一层由令牌版本号兜底，两者是不同层次的保险。
 */
@Slf4j
@Service
public class CacheService {
    ...
    /** Redis 当前是否可用(用于限流/版本比对等需明确判断的场景；不要用它做业务判断) */
    ...
    /** 自增(用于限流/令牌版本号)，返回自增后的值；Redis 不可用时返回 -1 表示"未计数" */
    ...
    /** SADD：加入集合；Redis 不可用时返回 -1(调用方放弃标记) */
