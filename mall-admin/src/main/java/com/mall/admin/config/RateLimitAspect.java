 ...
 * Redis 不可用或计数返回 -1 时放行（fail-open）。
 *
 * <p>本服务唯一的使用点是后台登录 {@code @RateLimit(scope="admin_login", limit=10, windowSeconds=60)}
 * —— 与单体逐字相同：**搬服务不许把撞库防护变松**。
 *
 * <p>身份维度：{@code By.USER} 时读网关注入的身份（{@link AdminIdentityResolver}），
 * 解析不到就回落客户端 IP —— 限流绝不能因为"拿不到身份"而放行。
 * 登录路径本来就在网关的放行名单里（没有身份），因此实际按 IP 计数（与单体一致）。
 */
@Slf4j
@Aspect
@Component
...
public class RateLimitAspect {
    ...
    @Around("@annotation(com.mall.admin.support.RateLimit)")
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        ...
        String identity = rl.by() == RateLimit.By.USER ? resolveAdmin(request) : clientIp(request);
    ...
    /** 身份维度：优先用网关注入的身份；解析不到（匿名/凭据不可信）→ 回落 IP */
    private String resolveAdmin(HttpServletRequest request) {
        try {
            return "a" + adminIdentityResolver.resolve(request).getId();
     ...
     * {@code X-Forwarded-For}/{@code X-Real-IP} 是客户端可以随便伪造的头，直接采信等于把限流开关交给攻击者
     * （每个请求换一个假 IP 就是全新的计数桶，撞库限流形同虚设）。
     * 部署在 Nginx 之后时把 {@code mall.rate-limit.trust-forwarded-for=true} 打开，
