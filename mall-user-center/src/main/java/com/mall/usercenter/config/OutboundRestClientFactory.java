...
/**
 * 出站 {@link RestClient} 的**统一装配**（P8-7 起，与 mall-admin 的同名类逐字一致，只改包名）。
 *
 * <h2>为什么要它</h2>
 * 每个出站客户端原先各写一遍这段装配（`lb://` 分支 / 直连分支 / 连接超时 / 读超时 / 内部令牌头），
 * 全项目一共 14 个客户端 ⇒ 14 份几乎相同的代码，且**直连分支容易漏东西**：
 ...
 * （真库桩测试就是走直连，一漏就被断言抓到：`Expecting actual: [null] to contain "test-internal-token"`）。
 *
 * <h2>两条分支的头处理（刻意不同，避免重复拦截）</h2>
 * <ul>
 *   <li>{@code lb://} ⇒ 用传入的 {@code @LoadBalanced} builder；它上面**已经**挂了
 *       {@code OutboundHeadersInterceptor}（见 {@link LoadBalancedClientConfig}），这里不再加第二个；</li>
 *   <li>{@code http://} ⇒ 普通 builder + **显式加同一个拦截器**，于是"直连排障模式"与"生产模式"
 *       发出的头完全一致（排障时不会因为少一个头而得到 401，把问题指向错误的方向）。</li>
 * </ul>
 *
 * <p>本服务的线格式契约测试 {@code ProductSnapshotClientTest} 走的正是 {@code http://} 直连分支，
 * 并逐条断言出站请求必须带 {@code X-Internal-Token} ⇒ 这个类也是那条断言的落点。
 */
@Component
