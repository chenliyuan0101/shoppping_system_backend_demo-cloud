...
/**
 * 方法调用调试日志切面：
 * 对 controller / internal / service / task 的所有方法打印 调用进出 + 参数摘要 + 耗时(debug 级别)。
 * 开启：application-dev.yaml logging.level.com.mall.search=debug（默认已开）。
 *
 * <p>⚠️ 与其它服务的切点**有一处刻意的差异**：这里多加了 {@code internal}。
 * 原因是本服务的**全部端点都在 {@code com.mall.search.internal}**（只有 {@code /internal/v1/**}），
 * 照抄 marketing 的三段切点（controller/service/task）会导致"请求一条都不打日志"，
 * 排查跨服务调用时反而少了一双眼睛。差异只影响 debug 日志覆盖面，不影响任何行为。
 */
@Aspect
@Component
@Slf4j
public class MethodLogAspect {
    ...
    @Around("execution(* com.mall.search..controller..*(..))"
            + " || execution(* com.mall.search..internal..*(..))"
            + " || execution(* com.mall.search..service..*(..))"
            + " || execution(* com.mall.search..task..*(..))")
